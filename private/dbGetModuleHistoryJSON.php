...
function ciniki_core_dbGetModuleHistoryJSON(&$ciniki, $module, $history_table, $tnid, $table_name, $table_key, $table_field, $json_field) {
    ...
    if( $result == false ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.core.415', 'msg'=>'Database Error', 'pmsg'=>mysqli_error($dh)));
    }
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.core.416', 'msg'=>'Unable to merge user information', 'err'=>$rc['err']));
    }
    $users = $rc['users'];
