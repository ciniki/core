...
//
function ciniki_core_dbGetModuleHistoryFlagBits(&$ciniki, $module, $history_table, $tnid, $table_name, $table_key, $table_field, $bits) {
    //
...
//      . "DATE_FORMAT(log_date, '" . ciniki_core_dbQuote($ciniki, $datetime_format) . "') AS date, "
    ...
    if( $result == false ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.core.413', 'msg'=>'Database Error', 'pmsg'=>mysqli_error($dh)));
    }
...
//      }
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.core.414', 'msg'=>'Unable to merge user information', 'err'=>$rc['err']));
    }
    $users = $rc['users'];
