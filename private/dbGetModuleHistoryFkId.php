...
//
function ciniki_core_dbGetModuleHistoryFkId(&$ciniki, $module, $history_table, $tnid, $table_name, $table_key, $table_field, $fk_table, $fk_id_field, $fk_value_field) {
    //
    ...
    if( $result == false ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.core.49', 'msg'=>'Database Error', 'pmsg'=>mysqli_error($dh)));
    }
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.core.50', 'msg'=>'Unable to merge user information', 'err'=>$rc['err']));
    }
    $users = $rc['users'];
