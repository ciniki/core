...
//
//
function ciniki_core_dbGetModuleHistoryNotes(&$ciniki, $module, $history_table, $business_id, $table_name, $table_key, $table_field) {
    //
...
//      . "DATE_FORMAT(log_date, '" . ciniki_core_dbQuote($ciniki, $date_format) . "') as date, "
    ...
    if( $result == false ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.core.399', 'msg'=>'Database Error', 'pmsg'=>mysqli_error($dh)));
    }
    ...
    while( $row = mysqli_fetch_assoc($result) ) {
        $rsp['history'][$num_history] = array('action'=>array('user_id'=>$row['user_id'], 'date'=>$row['date'], 'value'=>$row['value'], 'notes'=>$row['notes']));
        if( is_array($table_key) ) {
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.core.400', 'msg'=>'Unable to merge user information', 'err'=>$rc['err']));
    }
    $users = $rc['users'];
