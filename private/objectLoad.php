...
function ciniki_core_objectLoad(&$ciniki, $obj_name) {
    ...
    if( !file_exists($method_filename) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.core.107', 'msg'=>'Unable to load object definition: ' . $pkg . '.' . $mod . '.' . $obj));
    }

    require_once($method_filename);
    if( !is_callable($method_function) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.core.108', 'msg'=>'Unable to load object definition: ' . $pkg . '.' . $mod . '.' . $obj));
    }

    $rc = $method_function($ciniki);
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.core.109', 'msg'=>'Unable to load object definition: ' . $pkg . '.' . $mod . '.' . $obj));
    }
    if( !isset($rc['objects']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.core.110', 'msg'=>'Unable to load object definition: ' . $pkg . '.' . $mod . '.' . $obj));
    }
    $objects = $rc['objects'];

    if( !isset($objects[$obj]) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.core.111', 'msg'=>'Unable to load object definition: ' . $pkg . '.' . $mod . '.' . $obj));
    }
