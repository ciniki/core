...
//            lE = M.addSectionLabel(t, this.sectionCount(i, s));
    ...
    if( lE != null && (s.customer_buttons == null || s.customer_buttons == 'yes') && s.customer_id != null ) {
        ...
        var btn = M.aE('span', this.panelUID + '_' + i + '_menu_icon', 'faicon menu-icon', '&#xf142;', 'event.stopPropagation();' + this.panelRef + '.customerEditMenu(event,\'' + i + '\');');
        btns.appendChild(btn);
        lE.classList.add('buttons');
        lE.appendChild(btns);
    }
    ...
    // Add menu 
    //
    if( lE != null && s.menu != null ) {
        if( s.menu.visible == null || s.menu.visible() == 'yes' ) {
            var btns = M.aE('div', null, 'fabuttons');
            var btn = M.aE('span', this.panelUID + '_' + i + '_menu_icon', 'faicon menu-icon', '&#xf142;', 'event.stopPropagation();' + this.panelRef + '.sectionMenu(event,\'' + i + '\');');
            btns.appendChild(btn);
            lE.classList.add('buttons');
            lE.appendChild(btns);
        }
...
//    var dt = new Date();
    MG.data_graphic({
        ...
        top: 20,
        full_width: true,
        linked: (sc.linked != null ? sc.linked : false),
        max_y: (sc.max_y != null ? sc.max_y : null),
        min_y: (sc.min_y != null ? sc.min_y : null),
        min_y_from_data: (sc.min_y_from_data != null ? sc.min_y_from_data : null),
...
M.panel.prototype.showHideFormField = function(s, fid) {
    ...
    if( typeof this.sections[s].fields[fid].visible == 'function' && this.sections[s].fields[fid].visible() == 'no' ) {
        o.parentNode.style.display = 'none';
    } else if( this.sections[s].fields[fid].visible != null && this.sections[s].fields[fid].visible == 'no' ) {
        o.parentNode.style.display = 'none';
    } else {
...
//        f.setAttribute('onchange', this.panelRef + '.setFileName(\''+i+'\');');
        ...
        if( field.onclick != null && field.onclick != '' ) {
            onclick = 'onclick="' + field.onclick + '(event);" ';
        }
        if( this.imageURL != null ) {
            var url = this.imageURL(s, i, field, img_id, mN);
        } else {
...
M.panel.prototype.sectionMenu = function(event, s) {  
    var wrap = M.aE('div',null,'wrap');
    for(var i in this.sections[s].menu) {
        if( i == 'visible' ) {
            continue;
        }
...
M.panel.prototype.popupMenuClose = function(s) {
    var i = M.gE(this.panelUID + '_' + s + '_menu_icon');
    if( i != null ) {
        i.innerHTML = '&#xf142;';
    }
